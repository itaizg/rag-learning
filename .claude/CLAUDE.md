...
- 🚧 **Tier 10 — Classic ML (37–42) — PLANNED, not built (2026-08-01).** Scaffolded `10_classic_ml/` (section README with the paper→notebook table), registered in root README (tier list + triage table stub) and the Content Source Map above ("Classic ML Papers"). 6 notebooks planned from 8 papers (37 Domingos folk wisdom 🔴, 38 Efficient BackProp + Adam 🔴, 39 Random Forests + XGBoost 🔴, 40 sklearn 🟡, 41 SHAP 🟡, 42 Spark 🟢); Attention/BERT papers from the same batch are cross-links to 04/05/05b, not new notebooks. All fully offline. Build-time TODOs recorded in the plan: `uv add xgboost shap` (NOT pyspark — guarded optional cell only), **extend `notebooks.sh`/`lint.sh` globs from `0[0-9]_*` to cover `10_*`** (they currently would silently skip Tier 10), mermaid subgraph + fast-path prefix + 5th learner profile in README, BERT concept cell in 05b. Full plan: `~/.claude/plans/classic-ml-section.md`.
