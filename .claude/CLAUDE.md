...
- Wave 4 (2026-07-30) built the capstone: **`06_projects/P5_optimized_inference_service`** (🟡) composes prefix caching + admission control (13d), quantized model swap (10b), and cost/quality routing (32c) as four cumulative layers on ONE deliberately-overloaded mock service (an in-process `asyncio` timing model, not a real subprocess server — 29b/29c already teach that mechanism, so P5 reuses only the timing shape). A Poisson arrival process at 190 req/s against a ~127 req/s baseline capacity produces genuine sustained overload (P99 ~2.1s at baseline); each layer is re-benchmarked with a FIXED workload across 3 trials, reporting the median (the same distrust-a-single-number instinct as eval hygiene, applied to latency). The measured story: caching alone only partially helps (it removes prefill time, but decode dominates total latency here) → admission control caps P99 by shedding ~14% of requests outright → quantization fixes the actual capacity shortfall, and admission control's shed count drops to 0 (it becomes an idle safety net once the root cause is fixed, not a coincidence) → cost/quality routing spends some of the new headroom back on quality for the ~30% of requests that declared they needed it, more cheaply than blanket admission-control-without-routing. Model tiers reuse 32c's OWN measured quality/cost numbers (not new invented ones) for cross-notebook continuity. Final cell auto-generates `BENCHMARKS.md` at the repo root (methodology, config table, results table, honest limitations) — the publishable artifact, not just the notebook.
...
- **P2 reuses `ragkit` unmodified** (`load_corpus`, `chunk_text`, `build_collection`, `query_collection`, `generate`) — a capstone's job is composition + evaluation, not reinventing retrieval. **Correction (found while building P5, 2026-07-30):** the note previously here claimed nbclient runs with cwd = the notebook's own directory — that's wrong. `NotebookClient(...).execute()` with no explicit `resources` uses the cwd of the *process that invokes it* (verified empirically: a throwaway notebook printing `os.getcwd()`, run via `tools/run_nb.py` from the repo root, prints the repo root, not `06_projects/`). P2's `import ragkit` actually works because Python puts the process's cwd on `sys.path` by default and `ragkit/` lives directly in the repo root — the `sys.path.insert(0, "..")` line is harmless but was never the reason it worked. **Practical consequence:** any notebook writing files with a relative path (like P5's `open("BENCHMARKS.md", "w")`) must write relative to the REPO ROOT, not relative to its own tier directory — `deploy/observability/`, `deploy/k8s/`, and `deploy/webllm/` in Waves 2-3 all got this right by accident (no leading `../`); P5's first draft got it wrong (`open("../BENCHMARKS.md", ...)`, assuming cwd = `06_projects/`) and the file landed one directory ABOVE the repo root until caught by checking the file actually existed where expected, not just trusting the printed "Wrote ..." line.
...
- **CPU microbenchmarks on a shared/multi-tenant sandbox are noisy enough that a single-measurement claim doesn't reproduce.** An initial single-trial ORT-INT8-vs-ORT-FP32 benchmark in 37 showed a clean ~1.4-1.5x speedup; re-running the exact same code minutes later (still valid, just resampled) gave 1.08-1.23x on a `median`-of-5-trials methodology, and a bare single run without even that hygiene swung as low as 1.08x. Fixed two ways: (1) the benchmark helper itself now runs a warmup plus several independent timed trials and reports the **median** (the same distrust-a-single-number instinct notebook 25 teaches for eval scores, NOT a technique notebook 25 itself teaches — nb25 is about eval-data contamination, not latency measurement methodology; an earlier draft of this note and of 37's own prose both mischaracterized nb25 as teaching warmup/trials directly, fixed in both places 2026-07-30); (2) the surrounding prose was rewritten to state a defensible range ("roughly 1.1-1.5x, treat as directional") instead of a specific number, while keeping the ~4x file-size reduction — which is deterministic, not sampled — as the claim actually worth trusting. Don't tune prose to match one lucky run; either make the measurement robust or state the honest range.
```

### Inference-optimization conventions (Wave 4 / P5 capstone, added this build)
- **`nbclient`'s default execution cwd is the invoking PROCESS's cwd, not the notebook's own directory** — see the corrected Tier 6 capstone conventions bullet above (previously wrong on this point). Any notebook writing a file with a relative path must write it relative to wherever `tools/run_nb.py` is actually invoked from (the repo root, per this repo's documented tooling), not relative to the notebook's own tier folder. P5's first draft wrote `open("../BENCHMARKS.md", "w")` assuming cwd = `06_projects/`; the file silently landed one directory ABOVE the repo root (`/home/user/BENCHMARKS.md`) even though the cell's own printed "Wrote ../BENCHMARKS.md" line looked completely normal — caught only by explicitly checking the file existed at its claimed path with `ls`, not by trusting the print statement. Fixed to `open("BENCHMARKS.md", "w")`.
- **Verify a filtered/argmin result against the SAME numbers the filter uses, not against an assumption about which numbers are "obviously" cheap or fast.** P5's Exercise 3 solution first used an SLO of `quality >= 0.8`, which — checked against the actual measured `layer_results` — left only ONE candidate layer, and the accompanying comment claimed a completely different (and, on inspection, wrong) layer was the answer, while also asserting the baseline layers were "cheaper" when they were actually the MOST expensive (they always use the costly "full" tier for every request; being slow isn't the same as being cheap). Fixed by re-deriving the SLO threshold (0.7) empirically from the real numbers until it produced a genuinely instructive comparison (two real candidates, with the routed layer winning on cost), then rewriting the commentary to match what the code actually computes — the general lesson: run the exercise's own solution code and read its output before writing the prose that explains it, every time, even for "obvious" comparisons.
- **A composition capstone's backend can be simpler than the notebooks that taught its pieces, if that simplification is stated honestly.** P5 needed a service to layer four optimizations onto, but re-teaching 29b/29c's real FastAPI-subprocess mechanics wasn't the point — an in-process `asyncio.Semaphore`-bound timing model (same shape, no subprocess) was enough to produce genuine queueing/overload dynamics while keeping the notebook simple and fast. The key design choice that made the resulting story trustworthy: an **open-loop Poisson arrival process** (not a closed-loop fixed-worker loop) sized so the offered rate (190 req/s) genuinely exceeds the baseline's capacity ceiling (~127 req/s) — a closed-loop or under-provisioned open-loop test would never produce the sustained-overload-then-recovery arc the four layers are supposed to demonstrate. Verified by running the full 5-layer, 3-trial comparison twice independently before committing to the numbers in prose; both runs agreed to within a few percent on every metric.
