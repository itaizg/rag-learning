...
### CI/CD Eval Gate Policy
- Confident AI, ["Best AI Evaluation Tools for CI/CD"](https://www.confident-ai.com/knowledge-base/compare/best-ai-evaluation-tools-for-ci-cd) — a vendor comparison (Confident AI, Noveum, Promptfoo, MLflow 3, Arize Phoenix, LangSmith, Braintrust, Ragas, LangWatch) that ranks its own product first, but whose underlying gate-policy design is sound and citable independent of the ranking: benchmark composition (25–50 reviewed cases with provenance/reviewer status/policy slices), flakiness control (pin versions, explicit rubrics, lower temperature, **measure the unchanged system repeatedly before setting policy**), dual thresholds (absolute floor AND baseline delta — e.g. "score > 0.80 and no more than a 0.03 decline"), min-pass rules over repeated generations, slice checks, staged policy (PR critical / nightly variance / pre-release full), versioned release evidence.
- CircleCI, ["CI/CD Testing Strategies for Generative AI Apps"](https://circleci.com/blog/ci-cd-testing-strategies-for-generative-ai-apps/) — why deterministic CI breaks on GenAI (hallucination, bias, drift, latency) and six strategy families (hallucination detection, bias mitigation, model-graded evals, consistency/snapshot testing, performance monitoring, security/adversarial testing). Contains no code; its contribution is the failure-mode-to-check framing, since 5 of its 6 families were already taught (24/26/27c faithfulness, 26 judge bias, 27 drift, 13c/29 latency, 30 adversarial) before this notebook existed.
- Built as `08_production/33b_eval_gate_policy.ipynb` (🟡) + `agentkit/evalgate.py`, hardening `33_ci_for_ai.ipynb`'s single-sample, zero-tolerance `regression_gate` (no variance awareness, no absolute floor, no slice check) into a policy calibrated against measured noise.
...
- ✅ **Content-gap notebooks + `agentkit` — CLOSED (2026-07-27).** A coverage audit against a 22-topic "AI Engineer must-learn" list found 14 topics fully covered, 5 partial, 3 fully missing. Closed with a new top-level importable package (`agentkit/` — mirrors `ragkit/`, ~350 lines, pure numpy+stdlib, no torch/chroma/anthropic imports since embed fns are injected) plus 4 notebooks, each executed clean individually via `.claude/checks/notebooks.sh`: **`03_building/13c_inference_economics`** (🔴, prefill-vs-decode measured live on real GPT-2 on MPS via `torch.mps.synchronize()`, FP8/INT8/NF4 grid-error comparison, a *measured* "when quantization hurts" — naive INT4 raised GPT-2 perplexity ~24x, AWQ-style salient-channel protection recovered most of it — a real Hinton-distillation demo on sklearn moons, `agentkit.SemanticCache` built and used for the first time with its false-hit/staleness failure modes demonstrated on real Helios-corpus embeddings, and a capstone fine-tune/ICL/RAG/distillation decision framework), **`05_evaluation/24b_retrieval_and_citation_evals`** (🟡, precision@k/recall@k/nDCG@k with graded relevance built from scratch, a real embedding-vs-BM25 divergent-metric example found empirically over a hand-graded Helios chunk corpus, a stale-vs-fresh-chunk retrieval demo with a recency rescore, live citation-validity/support/coverage scoring), **`08_production/28b_tool_contracts_and_reliability`** (🔴, hardens nb19's bare `TOOL_IMPLEMENTATIONS[name](**args)` line with `agentkit.contracts.checked_execute` — hallucinated-tool detection, schema+semantic validation, a real idempotency-key proof against a `REFUND_LEDGER` double-fire, a measured retries-vs-fallback success-rate staircase 26%→58%→100% on a seeded flaky backend), **`08_production/30d_multi_tenant_isolation`** (🟡, the *same* `SemanticCache` from 13c reused as the cross-tenant attack surface — measured contamination persisting at 71% even at threshold=0.99, proving threshold tuning is not a structural fix — `NamespacedStore` contamination@k, a cross-user memory-leak demo, and `CostLedger` per-tenant-per-journey attribution wired into 28b's `checked_execute` via a `pre_hooks` budget gate). README triage tables (Tiers 3/5/8), mermaid map, and 🔴 fast path (added 13c, 28b) all updated. Full plan: `~/.claude/plans/write-a-plan-to-eager-flame.md`.
  - **`agentkit` design rule:** plumbing learners REUSE and exercises MUTATE goes in the package (`ToolContract`/`checked_execute`, `SemanticCache`, `TenantContext`/`NamespacedStore`/`CostLedger`); one-off demos, golden sets, and workloads stay in-notebook. One-directional interlock, no back-edges: `semcache` built in 13c → attack surface in 30d; `contracts` built in 28b → budget-gated in 30d via `CostLedger.make_budget_hook` as a `checked_execute` `pre_hook`; `gates` built in 27c → `evalgate` (33b) converts its `GateOutcome` into `gates.Evidence` via `gate_evidence()`, never the reverse. `24b` has no agentkit dependency at all.
...
- ✅ **CI/CD eval-gate policy — CLOSED (2026-08-12).** See Content Source Map > "CI/CD Eval Gate Policy" for both sources. `33_ci_for_ai.ipynb`'s `regression_gate` was a single-sample, zero-tolerance comparison with no absolute floor and no slice awareness — the exact weak pipeline the Confident AI piece warns about. Added `agentkit/evalgate.py` (~230 lines, stdlib+numpy only, one-directional import of `agentkit.gates` — no back-edge): `VarianceProfile`/`measure_variance` (repeat a suite against an unchanged system and summarize the spread), `ThresholdPolicy`/`evaluate_candidate` (absolute floor AND baseline delta, both checked independently and both reported so a gate never hides a second failure behind the first one it happened to catch; plus optional `min_pass_of_n` and `slice_floor`), `slice_means`, `StagePolicy`/`DEFAULT_STAGES`/`select_cases` (PR/nightly/pre-release coverage), `ReleaseEvidence` (JSON round-trip with a `dataset_hash` sha256 over the sorted case list — "was this the same benchmark?" as a check, not an assertion), and `gate_evidence()` converting a `GateOutcome` into `gates.Evidence` for `MergeGate`. New notebook `08_production/33b_eval_gate_policy.ipynb` (🟡, 34 cells): a seeded-RNG unchanged-system simulator shows a naive zero-tolerance gate flagging ~20% of clean runs as false regressions purely from noise on a 10-case suite, then calibrates a tolerance off the measured std; a live guarded judge-scored variance measurement reports the real observed spread (no scripted number); three hand-built candidates each fail (or pass) for exactly one named reason — baseline-delta only, absolute-floor only (checked against a second, already-marginal baseline so the delta genuinely doesn't fire), or neither; a min-pass-of-n demo isolates that rule from floor/delta by setting both deliberately loose; a slice demo where the aggregate (0.82) clears the floor while the `adversarial` slice (0.40) collapses, with a bar chart; staged coverage cost printed per trigger, a `ReleaseEvidence` JSON round-trip with hash re-verification, and a `MergeGate` hand-off. Section G folds in the source's 9-tool comparison as a table (with an explicit self-ranking caveat) plus two no-install shims — a `yaml`-parsed promptfoo-style config and a deepeval-style `assert_test` — run over this notebook's own harness rather than installed. Executed clean via both `.claude/checks/notebooks.sh` and `tools/run_nb.py`; `33_ci_for_ai.ipynb` re-verified undisturbed. README Tier 8 triage table, mermaid chain (`N33 → N33b`, cross-tier edge moved to `N33b → N34`) updated; fast path unchanged (🟡). **Build-time finding:** `pyyaml` is already a transitive dependency (via `chromadb`/`datasets`/`transformers`) — confirmed present in `uv.lock`, not added as a direct dependency; no `uv add` needed. Disposable `tools/build_33b.py` deleted after the notebook executed clean, per convention.
