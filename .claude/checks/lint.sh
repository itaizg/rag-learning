#!/usr/bin/env bash
# Goal check: every notebook code cell and every ragkit module parses.
...
# 2. ragkit package modules
for py in sorted(pathlib.Path("ragkit").rglob("*.py")):
    try:
...
if not fail:
    print("PASS lint: all notebook cells and ragkit modules parse")
sys.exit(fail)
PYEOF
